...
import type { Cart } from "@onlinesadar/shared";
...
interface CartContextValue {
  ...
  addItem: (productSlug: string, quantity?: number, options?: { isSample?: boolean }) => Promise<void>;
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addItem = async (productSlug: string, quantity = 1, options?: { isSample?: boolean }) => {
    const sid = resolveSessionId();
    ...
    const data = await api<{ cart: Cart }>("/cart/items", {
      ...
      sessionId: sid,
      token,
      body: JSON.stringify({ productSlug, quantity, isSample: options?.isSample }),
    });
    setCart(normalizeCart(data.cart));
